...
## What's new
- For more flexibility, the `RawJWT` option has been replaced with two options: `TokenAsPlainText` - when we get a non-JSON response, and `TokenExpiresFromJWT` - for retrieving the expiration date (works with both JSON and text response bodies)
## Features
...
- Option `TokenAsPlainText` - when the server returns only a token as plain text 
- Option `TokenExpiresFromJWT` when we have to extract the expiration date ourselves
